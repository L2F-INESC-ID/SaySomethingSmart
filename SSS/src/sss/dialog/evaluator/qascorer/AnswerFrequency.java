...
public class AnswerFrequency extends QaScorer {
    ...
    public AnswerFrequency(double weight, SimilarityMeasure similarityMeasure) {
    ...
    public void score(String userQuestion, List<QA> qas, Conversation conversation) {
        ...
        for (int i = 0; i < qas.size(); i++) {
            for (int j = i + 1; j < qas.size(); j++) {
                QA qa1 = qas.get(i);
                QA qa2 = qas.get(j);
                double score = getSimilarityMeasure().distance(qa1.getAnswerListNormalized(), qa2.getAnswerListNormalized());
                scores[i] += score;
                scores[j] += score;
            }
        }
